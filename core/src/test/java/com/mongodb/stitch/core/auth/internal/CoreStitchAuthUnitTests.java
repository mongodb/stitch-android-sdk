...
public class CoreStitchAuthUnitTests {
    ...
    public void testLoginWithCredentialBlocking() {
        ...
        final CoreStitchUser user =
                auth.loginWithCredentialBlocking(
                        new CoreAnonymousAuthProviderClient().getCredential()
                );
    ...
    public void testLinkUserWithCredentialBlocking() {
        ...
        final CoreStitchUser linkedUser = auth.linkUserWithCredentialBlocking(
                user,
    ...
    public void testHandleAuthFailure() {
        final StitchRequestClient requestClient = getMockedRequestClient();
        final StitchAuth auth = Mockito.spy(new StitchAuth(
                requestClient,
                new TestAuthRoutes(),
                new MemoryStorage()));
        ...
        doThrow(new StitchServiceException(StitchServiceErrorCode.INVALID_SESSION))
                .doReturn(new Response(getMockedAuthInfoResponse(DEFAULT_ACCESS_TOKEN, DEFAULT_REFRESH_TOKEN)))
                .when(requestClient).doRequest(argThat(req -> req.path.endsWith("/link")));

        user = auth.linkUserWithCredentialBlocking(
                user,
                new UserPasswordCredential(
                        CoreUserPasswordAuthProviderClient.DEFAULT_PROVIDER_NAME,
                        "foo@foo.com",
    ...
    public void testDoAuthenticatedJSONRequestWithDefaultCodecRegistry() {
        ...
        final String docRaw = String.format("{\"_id\": {\"$oid\": \"%s\"}, \"intValue\": {\"$numberInt\": \"42\"}}", expectedObjectId.toHexString());
        ...
        doReturn(new Response(docRaw)).when(requestClient).doRequest(any());
        final CustomType ct = auth.doAuthenticatedJSONRequest(reqBuilder.build(), new CustomType.Codec());
        ...
        final List<Object> arrFromServer = Arrays.asList(21, "the meaning of life, the universe, and everything", 84, 168);
...
//
    ...
    StitchRequestClient getMockedRequestClient() {
        ...
        when(requestClient.doJSONRequestRaw(any())).thenAnswer(inv -> requestClient.doRequest(inv.getArgument(0)));
        ...
        doReturn(new Response(getMockedAuthInfoResponse(accessToken, refreshToken)))
                .when(requestClient).doRequest(argThat(req -> req.path.endsWith("/login")));

        // Profile works if the access token is the same as the above
        doAnswer(inv -> {
            final StitchRequest req = inv.getArgument(0);
            assertEquals("Bearer " + accessToken, req.headers.getOrDefault("Authorization", ""));
            return new Response(getMockedUserProfile());
        }).when(requestClient).doRequest(argThat(req -> req.path.endsWith("profile")));

        // Link works if the access token is the same as the above
        doAnswer(inv -> {
            final StitchRequest req = inv.getArgument(0);
            assertEquals("Bearer " + accessToken, req.headers.getOrDefault("Authorization", ""));
            return new Response(getMockedAuthInfoResponse(accessToken, refreshToken)); // TODO: Really is different
        }).when(requestClient).doRequest(argThat(req -> req.path.endsWith("/link")));
