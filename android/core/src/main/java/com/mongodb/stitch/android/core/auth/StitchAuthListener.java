...
public interface StitchAuthListener {
  ...
  /**
   * onAuthEvent is called any time a notable event regarding authentication happens. These events
   * are: * When a user logs in. * When a user logs out. * When a user is linked to another
   * identity. * When a listener is registered. This is to handle the case where during registration
