...
public class GCMPushClient extends PushClient {
    ...
    public GCMPushClient(
        ...
        if (info.isFromProperties()) {
            ...
            if (!baasClient.getProperties().containsKey(PROP_GCM_SERVICE_NAME)) {
                throw new BaasException.BaasClientException("No GCM service name in properties");
            }
            final String serviceName = baasClient.getProperties().getProperty(PROP_GCM_SERVICE_NAME);
            final String senderId = baasClient.getProperties().getProperty(PROP_GCM_SENDER_ID);
