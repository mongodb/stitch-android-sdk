...
class SyncMongoClientIntTests : BaseStitchServerIntTest() {
    ...
    fun describe(description: String, testFun: () -> Unit) {
        println(description)
        testFun()
    }

    fun it(should: String, testFun: () -> Unit) {
        println("...should $should")
        testFun()
    }

    @Test
    fun testLocalToRemote() {
        val localColl = getTestColl()
        localColl.configure({
        ...
        localColl.insertOneAndSync(doc1)

        goOnline()

        describe("each document that has local writes pending") {
            it("retrieve the synthetic change event for this local document in the local config metadata") {
                syncPass()
            }
        }
    }
