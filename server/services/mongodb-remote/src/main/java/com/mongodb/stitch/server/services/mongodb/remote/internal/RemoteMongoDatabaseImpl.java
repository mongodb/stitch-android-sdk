...
public final class RemoteMongoDatabaseImpl implements RemoteMongoDatabase {
  ...
  RemoteMongoDatabaseImpl(final CoreRemoteMongoDatabase database) {
    this.proxy = database;
  }
  ...
  public RemoteMongoCollection<Document> getCollection(final String collectionName) {
    return new RemoteMongoCollectionImpl<>(proxy.getCollection(collectionName));
  }
  ...
  public <DocumentT> RemoteMongoCollection<DocumentT> getCollection(
    ...
    return new RemoteMongoCollectionImpl<>(proxy.getCollection(collectionName, documentClass));
  }
}
