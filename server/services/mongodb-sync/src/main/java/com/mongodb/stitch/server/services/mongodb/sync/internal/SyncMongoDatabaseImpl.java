...
public final class SyncMongoDatabaseImpl implements SyncMongoDatabase {
  ...
  SyncMongoDatabaseImpl(final CoreSyncMongoDatabase database) {
    this.proxy = database;
  }
  ...
  public SyncMongoCollection<Document> getCollection(final String collectionName) {
    return new SyncMongoCollectionImpl<>(proxy.getCollection(collectionName));
  }
  ...
  public <DocumentT> SyncMongoCollection<DocumentT> getCollection(
    ...
    return new SyncMongoCollectionImpl<>(proxy.getCollection(collectionName, documentClass));
  }
